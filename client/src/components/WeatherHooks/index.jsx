...
const WeatherHooks = () => {
  ...
  const {cityName, weather, description, windSpeed} = fetchWeather(`https://api.openweathermap.org/data/2.5/weather?q=${finalCity},uk&appid=${weatherKey}`, loading)
  ...
  return (
    <div>
      ...
      <div>{!finalCity ? 'loading city data' : cityName}</div>
    </div>
  )
}
